...
# Masks for 64-bit bitboards and accessing specific rows.
...
def board_is_initial_removal_phase(blackBoard: int, whiteBoard: int) -> bool:
    """
    If the game still has 63 or more pieces, the game is in the initial removal phase.
    Returns True if the board has over 63 pieces.
    """
...
def board_legal_removals(player: str) -> List[Move]:
    """
    Returns Moves representing the removals the given player (B or W) may take.
    The first player gets to take one of the center 4 pieces, then the other player must take an adjacent piece.
    """
    if player == BOARD_BLACK:
        #remove d5 or e4
...
def board_legal_jumps(blackBoard: int, whiteBoard: int, player: str) -> List[Move]:
    ...
    # Opponent bit shift, mask, row delta, and column delta (amount they moved)
    ...
    for l_shift, mask, d_row, d_col in directions:
        ...
        # opp_bits << l_shift checks if there's an opponent in the path.
        # empty << l_shift * 2 ensures there's an empty space on the other side.
        # l_shift is the amount to shift the opponent board by.
        # d_row is the number of rows moved across (vertical distance travelled)
        # d_col is the number of columns moved across (horizontal distance travelled)
